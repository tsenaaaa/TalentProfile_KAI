...
with col1:
    st.image("logo_kai.png", width=60)
with col2:
    st.markdown("<div class='big-title'>Profil Staff PT Kereta Api Indonesia</div>", unsafe_allow_html=True)
with col3:
    st.write("")
    ...
    if "Nama" in df_cleaned.columns:
        ...
        if generate_pdf:
            ...
            class CustomPDF(FPDF):
                ...
                def draw_behavior_competencies(self, bumn_list, multirater_list):
                    self.set_font('DejaVu', 'B', 10)
                    self.cell(0, 10, "BEHAVIOUR COMPETENCIES", ln=True, align='C')
                    
                    col_width = (self.w - 40) / 2  # Dua kolom seimbang
                    self.set_font('DejaVu', 'B', 9)
                    y_before = self.get_y()
                    self.multi_cell(col_width, 6, "BUMN", border=1, align='C')
                    y_after = self.get_y()
                    
                    self.set_xy(10 + col_width, y_before)
                    self.multi_cell(col_width, 6, "MULTIRATER", border=1, align='C')
                    
                    max_rows = max(len(bumn_list), len(multirater_list))
                    ...
                    for i in range(max_rows):
                        ...
                        bumn_text = bumn_list[i] if i < len(bumn_list) else ""
                        multirater_text = multirater_list[i] if i < len(multirater_list) else ""

                        # Save x/y before drawing
                        x_bumn = 10
                        x_multirater = 10 + col_width
                        y_bumn = self.get_y()

                        # Draw left cell
                        self.set_xy(x_bumn, y_bumn)
                        self.multi_cell(col_width, 6, bumn_text, border=1)

                        y_after_bumn = self.get_y()

                        # Draw right cell, align top
                        self.set_xy(x_multirater, y_bumn)
                        self.multi_cell(col_width, 6, multirater_text, border=1)

                        y_after_multirater = self.get_y()
                        y_next = max(y_after_bumn, y_after_multirater)
